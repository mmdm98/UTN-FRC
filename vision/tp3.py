...
cv2.imshow('Marcar 4 Puntos', param[1])
#activo callback y le paso la funcion + la original y la copia || preguntar porque anda acá y no necesariamente en el while*
cv2.setMouseCallback('Marcar 4 Puntos', draw4dots, param)

#loop
while (1):
    cv2.imshow('Marcar 4 Puntos', param[1])                             #siempre muestro la copia, en donde trabajo
    k = cv2.waitKey(1) & 0xFF                                           #cosas de teclado
    if k == ord('g'):                                                   #ord es para que agarre bien
        points2 = np.array([[0, 0], [img_aux.shape[1], 0], [0, img_aux.shape[1]], [img_aux.shape[0], img_aux.shape[1]]]).astype(np.float32)  #guardo cuatro puntos (float) de la img_aux
