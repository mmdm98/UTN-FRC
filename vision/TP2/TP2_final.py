...
coords = [[-1,-1],[-1,-1],[-1,-1]] #coordenadas iniciales

#funcion para dibujar 3 puntos que se guardan
def draw3dots(event, x, y, flags, param):
    global count, points, coords #toma el valor de las variables globales
    if event == cv2.EVENT_LBUTTONDOWN:
        if (count == 3):  #si cuente tres circulos reseteo el mundo
...
while (1):
    ...
    if k == ord('g'):                                                   #ord es para que agarre bien
        ...
        warp_mat = cv2.getAffineTransform(points2, points)  #se obtiene matriz que deforma de los tres puntos de img_aux, a los tres puntos click
        warp_dst = cv2.warpAffine(img_aux, warp_mat, (H,W)) #aplico la transformada afin a la img_aux y la muestro
        cv2.imshow('warpeada', warp_dst)
        overlay(warp_dst, param[1]) #funcion que come warpeada y copia y deshecha warpeada+copia
